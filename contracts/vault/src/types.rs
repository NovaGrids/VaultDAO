...
pub struct OracleConfig {
    ...
    /// Minimum number of data sources required
    pub min_sources: u32,
    /// Enable USD-denominated limits
    pub usd_limits_enabled: bool,
}

/// Price feed data from oracle
#[contracttype]
#[derive(Clone, Debug)]
pub struct PriceFeed {
    /// Token address
    pub token: Address,
    /// Price in USD (7 decimals, e.g., 1.5000000 = $1.50)
    pub price: i128,
    /// Ledger sequence when price was updated
    pub last_update: u64,
...
pub struct PortfolioValuation {
    /// Total value in USD (7 decimals)
    pub total_usd: i128,
    /// Individual token valuations
    pub token_values: Vec<TokenValue>,
    /// Ledger when valuation was calculated
    pub calculated_at: u64,
}

/// Individual token value in portfolio
#[contracttype]
#[derive(Clone, Debug)]
pub struct TokenValue {
    ...
    /// Token balance
    pub balance: i128,
    /// Price in USD (7 decimals)
    pub price: i128,
    /// Value in USD (7 decimals)
    pub value_usd: i128,
}
...
pub enum Condition {
    ...
    DateBefore(u64),
    /// Execute only when token price is above threshold (in USD, 7 decimals)
    PriceAbove(Address, i128),
    /// Execute only when token price is below threshold (in USD, 7 decimals)
