...
const ProposalListWidget: React.FC<ProposalListWidgetProps> = ({ widget, onRemove, onConfigure, isEditMode }) => {
  const { title, config } = widget;
  const proposals = (config.proposals as { id: string; title: string; status: string }[]) || [];
  ...
  return (
    <div className="h-full bg-gray-800 border border-gray-700 rounded-lg p-4 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-2">
        {proposals.map((p) => (
          <div key={p.id} className="p-2 bg-gray-900 rounded border border-gray-700">
