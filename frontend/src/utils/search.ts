...
/** Highlight search terms in text (case-insensitive, escape HTML, bold match) */
...
export function filterValuesToRecord(values: any[]): Record<string, any> {
  ...
  values.forEach((f) => {
    if (f.type === "text" && f.text != null && f.text !== "")
      obj[f.field] = f.text;
    if (f.type === "number_range") {
    ...
    if (f.type === "select" && f.select != null && f.select !== "")
      obj[f.field] = f.select;
    if (f.type === "multi_select" && f.multiSelect?.length)
      obj[f.field] = f.multiSelect;
  });
  return obj;
}
