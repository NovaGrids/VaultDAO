...
exports.BatchProposalOrchestrator = void 0;
exports.createBatchOrchestrator = createBatchOrchestrator;
 ...
 */
class BatchProposalOrchestrator {
    /**
     ...
     */
    addCreatedProposalId(proposalId) {
     ...
     */
    addCreatedProposalIds(proposalIds) {
     ...
     */
    async createProposals(proposerPublicKey) {
        const txnHashes = [];
        for (const transfer of this.state.transfers) {
            try {
                const txn = await this.retryOperation(async () => {
                    return await (0, contract_js_1.proposeTransfer)(proposerPublicKey, transfer.recipientPublicKey, transfer.tokenAddress, transfer.amount, transfer.description ?? "", this.options);
     ...
     */
    async approveAllProposals(signerPublicKey) {
        let successCount = 0;
        for (const proposalId of this.state.createdProposalIds) {
            try {
                await this.retryOperation(async () => {
                    await (0, contract_js_1.approveProposal)(signerPublicKey, BigInt(proposalId), this.options);
     ...
     */
    async approveProposal(signerPublicKey, proposalId) {
        try {
            await this.retryOperation(async () => {
                await (0, contract_js_1.approveProposal)(signerPublicKey, typeof proposalId === "string" ? BigInt(proposalId) : proposalId, this.options);
            }, `approve-proposal-${proposalId}`);
     ...
     */
    async executeAllProposals(executorPublicKey) {
        const executedIds = [];
        for (const proposalId of this.state.createdProposalIds) {
            try {
                await this.retryOperation(async () => {
                    await (0, contract_js_1.executeProposal)(executorPublicKey, BigInt(proposalId), this.options);
     ...
     */
    async executeProposal(executorPublicKey, proposalId) {
        try {
            await this.retryOperation(async () => {
                await (0, contract_js_1.executeProposal)(executorPublicKey, typeof proposalId === "string" ? BigInt(proposalId) : proposalId, this.options);
            }, `execute-proposal-${proposalId}`);
            const proposalIdStr = String(proposalId);
     ...
     */
    async executeFullOrchestration(proposerPublicKey, approverPublicKey, executorPublicKey) {
...
exports.BatchProposalOrchestrator = BatchProposalOrchestrator;
/**
 ...
 */
function createBatchOrchestrator(opts, retryConfig) {
